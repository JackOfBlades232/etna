...
static constexpr auto VK_BUFFER_ALIGNMENT = vk::DeviceSize{16};
...
PerFrameTransferHelper::PerFrameTransferHelper(CreateInfo info)
  ...
  , stagingSize{align_down(info.totalStagingSize / info.wc->multiBufferingCount(), VK_BUFFER_ALIGNMENT)}
  ...
  vk::CommandBuffer cmd_buf, const Buffer& dst, uint32_t offset, std::span<std::byte const> src)
{
  ...
  vk::DeviceSize stagingOffset = align_up(curFrameStagingOffset, VK_BUFFER_ALIGNMENT);
  ...
  vk::CommandBuffer cmd_buf, AsyncBufferUploadState& state)
{
  ...
  vk::DeviceSize stagingOffset = align_up(curFrameStagingOffset, VK_BUFFER_ALIGNMENT);
  size_t transferSize =
    std::min(state.src.size(), align_down(stagingSize - stagingOffset, VK_BUFFER_ALIGNMENT));
  if (transferSize == 0)
    return false;
  ...
  vk::CommandBuffer cmd_buf, AsyncBufferReadbackState& state)
{
  ...
  for (auto& issue : state.issues)
  {
    if (issue.fulfillmentFrame == wc.batchIndex())
    {
      ...
      ETNA_ASSERT(issue.offset == state.issuedOffset);
  ...
  vk::DeviceSize stagingOffset = align_up(curFrameStagingOffset, VK_BUFFER_ALIGNMENT);
  size_t transferSize =
    std::min(state.dst.size(), align_down(stagingSize - stagingOffset, VK_BUFFER_ALIGNMENT));
  if (transferSize == 0)
    return false;
  ...
  issue.offset = state.remainingOffset;
  ...
  size_t size)
{
  ...
  vk::Offset3D finalOffset =
    linear_to_offset3d(offset3d_to_linear(offset, imageExtent) + pixelsLeft, imageExtent);
